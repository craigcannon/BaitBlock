! function (a) {
    a.Parse = a.Parse || {}, a.Parse.VERSION = "js1.2.12"
}(this),
function () {
    var a = this,
        b = a._,
...
function (a) {
    ...
    b.serverURL = "https://api.parse.com", "undefined" != typeof process && process.versions && process.versions.node && (b._isNode = !0), b.initialize = function (a, c, d) {
        ...
        if ("undefined" != typeof XDomainRequest) return b._ajaxIE8(a, c, d)._thenRunCallbacks(g);
        var h = new b.Promise,
            i = !1,
        ...
        if ("batch" !== c && "classes" !== c && "events" !== c && "files" !== c && "functions" !== c && "login" !== c && "push" !== c && "requestPasswordReset" !== c && "users" !== c && "jobs" !== c) throw "Bad route: '" + c + "'.";
        var i = b.serverURL;
        "/" !== i.charAt(i.length - 1) && (i += "/"), i += "1/" + c, d && (i += "/" + d), e && (i += "/" + e), h = b._.clone(h || {}), "POST" !== f && (h._method = f, f = "POST"), b._.isUndefined(g) && (g = b._useMasterKey), h._ApplicationId = b.applicationId, g ? h._MasterKey = b.masterKey : h._JavaScriptKey = b.javaScriptKey, h._ClientVersion = b.VERSION, h._InstallationId = b._getInstallationId();
        var j = b.User.current();
        j && j._sessionToken && (h._SessionToken = j._sessionToken);
        var k = JSON.stringify(h);
        return b._ajax(f, i, k).then(null, function (a) {
            var c;
            if (a && a.responseText) try {
                var d = JSON.parse(a.responseText);
                d && (c = new b.Error(d.code, d.error))
            } catch (e) {}
            return c = c || new b.Error(-1, a.responseText), b.Promise.error(c)
        })
...
function (a) {
    ...
    b.Analytics = b.Analytics || {}, c.extend(b.Analytics, {
        track: function (a, d) {
            if (!a || 0 === a.trim().length) throw "A name for the custom event must be provided";
...
function (a) {
    ...
    var b = a.Parse,
        ...
        d = function (a) {
            ...
            var d = new b.Promise;
            if ("undefined" == typeof FileReader) return b.Promise.error(new b.Error(-1, "Attempted to use a FileReader on an unsupported browser."));
            var e = new FileReader;
            return e.onloadend = function () {
                if (2 !== e.readyState) return d.reject(new b.Error(-1, "Error reading file.")), void 0;
                var a = e.result,
                    f = /^data:([^;]*);base64,(.*)$/.exec(a);
                return f ? (d.resolve(f[2], c || f[1]), void 0) : (d.reject(new b.Error(-1, "Unable to interpret data URL: " + a)), void 0)
            }, e.readAsDataURL(a), d
        };
    b.File = function (a, d, h) {
        ...
        else if (d && d.base64) this._source = b.Promise.as(d.base64, j);
...
function (a) {
    ...
    b.Object = function (a, d) {
        ...
        _finishFetch: function (a, c) {
            this._opSetQueue = [{}], this._mergeMagicFields(a);
            var d = this;
            b._objectEach(a, function (a, c) {
                d._serverData[c] = b._decode(c, a)
            }), this._rebuildAllEstimatedData(), this._refreshCache(), this._opSetQueue = [{}], this._hasData = c
        },
        ...
        validate: function (a) {
            return !c.has(a, "ACL") || a.ACL instanceof b.ACL ? !1 : new b.Error(b.Error.OTHER_CAUSE, "ACL must be a Parse.ACL.")
        },
        ...
        if (c.has(b.Object._classMap, a)) {
            ...
            f = g._extend(d, e)
        } else d = d || {}, d.className = a, f = this._extend(d, e);
...
function (a) {
    ...
    b.User = b.Object.extend("_User", {
        ...
        current: function () {
            ...
            var c = JSON.parse(a);
            return b.User._currentUser.id = c._id, delete c._id, b.User._currentUser._sessionToken = c._sessionToken, delete c._sessionToken, b.User._currentUser.set(c), b.User._currentUser._synchronizeAllAuthData(), b.User._currentUser._refreshCache(), b.User._currentUser._opSetQueue = [{}], b.User._currentUser
        },
...
function (a) {
    ...
    b.Query = function (a) {
        ...
        get: function (a, c) {
            var d = this;
            return d.equalTo("objectId", a), d.first().then(function (a) {
                if (a) return a;
                var c = new b.Error(b.Error.OBJECT_NOT_FOUND, "Object not found.");
                return b.Promise.error(c)
            })._thenRunCallbacks(c, null)
        },
        toJSON: function () {
            var a = {
                where: this._where
            };
            return this._include.length > 0 && (a.include = this._include.join(",")), this._select && (a.keys = this._select.join(",")), this._limit >= 0 && (a.limit = this._limit), this._skip > 0 && (a.skip = this._skip), void 0 !== this._order && (a.order = this._order), b._objectEach(this._extraOptions, function (b, c) {
                a[c] = b
            }), a
        },
        find: function (a) {
...
function (a) {
    ...
    b.Installation = b.Object.extend("_Installation"), b.Push = b.Push || {}, b.Push.send = function (a, c) {
        if (c = c || {}, a.where && (a.where = a.where.toJSON().where), a.push_time && (a.push_time = a.push_time.toJSON()), a.expiration_time && (a.expiration_time = a.expiration_time.toJSON()), a.expiration_time && a.expiration_time_interval) throw "Both expiration_time and expiration_time_interval can't be set";
        var d = b._request({
            route: "push",
            method: "POST",
            data: a,
